...
<?php
require_once("../connect.php");
 $sql = "select auto, `Imie-Nazwisko`, (`WDW`.ID) as ID_TAB from `aaaaaaa_tojestbazadanych`.WDW, `aaaaaaa_tojestbazadanych`.auta, `aaaaaaa_tojestbazadanych`.Osoby where Osoby.ID=osoby_id and auta.id=klasa_id order by ID_TAB asc";
if ($conn->connect_error) {
die("Connection failed: " . $conn->connect_error);
}
$result = mysqli_query($conn, $sql);
